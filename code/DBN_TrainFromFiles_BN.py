...
if len(sys.argv) != 12:
    print ('Usage: DBN_TrainFromFiles.py learning_rate N#HiddenLayers N#HiddenUnits N#Outputs BatchSize N#Epochs OutFile AttributeIndx NumFrames BNhl BNhu')
    sys.exit(1)
#Parameters
lr = float(sys.argv[1])
...
iBNhl = int(sys.argv[10])
iBNhu = int(sys.argv[11])
...
def test_DBN(finetune_lr=lr, pretraining_epochs=100,
    ...
    h_l_Ss = [n_hus for i in range(n_hls)]
    h_l_Ss[iBNhl] = iBNhu
    dbn = DBN(numpy_rng=numpy_rng, n_ins=n_inp,
              hidden_layers_sizes=h_l_Ss,
              n_outs=n_out)
...
#
 ...
 #                         ' ran for %.2fm' % ((end_time - start_time) / 60.)))
    ...
    if not os.path.isdir('Temp_BN'):
        os.makedirs('Temp_BN')
    fbestParam = gzip.open('Temp_BN/Bestparam_'+str(iAttributeIndx)+'_'+str(iNumFrames)+'_'+str(n_hls)+'_'+str(n_hus)+'_'+str(n_out)+'_'+str(iBNhu)+'.pkl.gz','wb')
    fbestP = open('Temp_BN/Bestp_'+str(iAttributeIndx)+'_'+str(iNumFrames)+'_'+str(n_hls)+'_'+str(n_hus)+'_'+str(n_out)+'_'+str(iBNhu)+'.txt','w')
