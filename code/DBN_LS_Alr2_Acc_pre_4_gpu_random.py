...
from logistic_sgd_joblib_r import LogisticRegression, load_data
...
def test_DBN(finetune_lr=lr, pretraining_epochs=100,
    ...
    """
    try:
        fg = gzip.open(dataset,'rb')
        tr,vald,tst = cPickle.load(fg)
    except IOError:
        tr,vald,tst = joblib.load(dataset)
    y = tst[1]
    fg.close()
    ...
    #Randmize train
    #train_set_x_np = train_set_x.get_value()
    #train_set_y_np = train_set_y.get_value()
    #I = numpy.arange(train_set_x_np.shape[0])
    #numpy.random.shuffle(I)
    #train_set_x.set_value(train_set_x_np[I])
    #train_set_y.set_value(train_set_y_np[I])
    # compute number of minibatches for training, validation and testing
...
#
 ...
 #                         ' ran for %.2fm' % ((end_time - start_time) / 60.)))
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                if lossratio <= 0.0:
                    ...
                    yP = gety_pred()
                    #numpy.savetxt('Yp.txt',yP,fmt='%d')
                    #y = numpy.loadtxt(testFile,dtype=int)
                    #print (type(yP))
                    ...
                    if n_out == 3:
                        ...
                        acc3 = float(float(I33[0].size)/float(I3[0].size))
                        acc4 = float(float(I44[0].size)/float(I4[0].size))
                        acc5 = float(float(I55[0].size)/float(I5[0].size))
                    else:
                        acc3 = 0
                        acc4 = 0
                        acc5 = 0
    ...
    #                                          / 60.))
    fbestParam = gzip.open('Temp/Bestparam_'+DataSet+'_'+str(n_hls)+'_'+str(n_hus)+'_'+str(n_out)+'.pkl.gz','wb')
    fbestP = open('Temp/Bestp_'+DataSet+'_'+str(n_hls)+'_'+str(n_hus)+'_'+str(n_out)+'.txt','w')
    numpy.savetxt(fbestP,ybest)
    fbestP.close()
    cPickle.dump(best_params,fbestParam)
    fbestParam.close()
    OF = open(outFile,'a')
    print(DataSet, lr ,n_hls, n_hus, n_out, batchSize, NOfepoch, n_inp, outFile, test_score * 100., acc1 * 100., acc2 * 100., acc3 * 100, acc4 * 100, acc5 * 100, I11[0].size, I12[0].size, I13[0].size, I14[0].size, I15[0].size, I21[0].size, I22[0].size, I23[0].size, I24[0].size, I25[0].size, I31[0].size, I32[0].size, I33[0].size, I34[0].size, I35[0].size, I41[0].size, I42[0].size, I43[0].size, I44[0].size, I45[0].size, I51[0].size, I52[0].size, I53[0].size, I54[0].size, I55[0].size, file = OF)
