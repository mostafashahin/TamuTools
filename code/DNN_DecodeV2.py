    ...
    def build_finetune_functions(self, datasets, batch_size):
        ...
        def yPred():
            ...
            for i in xrange(n_test_batches):
                yP = numpy.concatenate((yP,yPred_i(i)))
            return yP
        def Py_given_x(n_out):
...
def Decode(n_hus, n_hls, n_out, arFeatures, sParamFile, iBNhl = -1, iBNhu = -1, vLabels = None, batch_size=100, bGet_yP=False):
    ...
    """
    n_samples,n_inp = arFeatures.shape
    shared_x = theano.shared(numpy.asarray(arFeatures, dtype=theano.config.floatX), borrow=True)
    if vLabels == None:
        vLabels = numpy.zeros(n_samples)
    shared_y = theano.shared(numpy.asarray(vLabels, dtype=theano.config.floatX), borrow=True)
