...
class DBN(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None, n_ins=n_inp,
        ...
        self.tanh_y = self.logLayer.tanh_y_given_x
    ...
    def build_finetune_functions(self, datasets, batch_size):
        ...
        test_score_i = theano.function([index], [self.errors, self.y_pred,self.p_y,self.tanh_y],
        ...
        def Py_given_x():
            ...
            return Py
        def PyP():
            PyX = numpy.empty((1,n_out))
            TyX = numpy.empty((1,n_out))
            yP = numpy.asarray([])
            for i in xrange(n_test_batches):
                print(i)
                testa = test_score_i(i)
                yP = numpy.concatenate((yP,testa[1]))
                PyX = numpy.concatenate((PyX,testa[2]))
                TyX = numpy.concatenate((TyX,testa[3]))
            PyX = numpy.delete(PyX,0,axis=0)
            TyX = numpy.delete(TyX,0,axis=0) 
            return yP,PyX,TyX
        return test_score, yPred, Py_given_x, PyP
...
def test_DBN(dataset=DataSet, batch_size=batchSize):
    ...
    test_model, gety_pred, getP_y, getPyP = dbn.build_finetune_functions(
    ...
    #numpy.savetxt(suffix+'_p_y_x',numpy.c_[Py,yP])
    joblib.dump(numpy.c_[Py,yP],suffix+'_p_y_x.jbl')
    joblib.dump(numpy.c_[Ty,yP],suffix+'_T_y_x.jbl')
