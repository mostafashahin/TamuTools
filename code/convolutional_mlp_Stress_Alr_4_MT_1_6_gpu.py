...
#    print (x.shahpe)
    ...
    cl2x = (nFB - fsx + 1)/p
    cl2y = (nFs - fsy + 1)/p
    layer1H = HiddenLayer(rng, input = layer1H_input, n_in = 27, n_out = nhu1/4, activation = T.tanh)
    ...
    layer2 = HiddenLayer(rng, input=layer2_inputT, n_in=(nkerns[0] * hl1 * 1)+nhu1/4,
                         n_out=nhu1*2, activation=T.tanh)

    layer22 = HiddenLayer(rng, input=layer2.output, n_in=nhu1*2,
                         n_out=nhu1, activation=T.tanh)

    layer23 = HiddenLayer(rng, input=layer22.output, n_in=nhu1,
                         n_out=nhu1, activation=T.tanh)


    layer24 = HiddenLayer(rng, input=layer23.output, n_in=nhu1,
                         n_out=nhu1, activation=T.tanh)


    layer25 = HiddenLayer(rng, input=layer24.output, n_in=nhu1,
                         n_out=nhu1, activation=T.tanh)


    layer26 = HiddenLayer(rng, input=layer25.output, n_in=nhu1,
                         n_out=nhu1, activation=T.tanh)

    # classify the values of the fully-connected sigmoidal layer
    layer3 = LogisticRegression(input=layer26.output, n_in=nhu1, n_out=n_out)
    ...
    params = layer3.params + layer26.params + layer25.params + layer24.params + layer23.params + layer22.params + layer2.params + layer1H.params + layer0.params
